...
//
// D-F9(b) (binding): per-agent records (run_status.agents / run_agent_log) survive server
...
// restart, run_status(runId).agents for a run whose agent completed BEFORE the restart comes
...
// notwithstanding), and run_agent_log on that same pre-restart agent returns AGENT_NOT_FOUND —
// McpFacade.run_agent_log (mcp-facade.ts:135-137) gates access on `view.agents.find(...)`
...
describe('Per-agent records survive a real server restart (IT-020, D-F9b)', () => {
  it('run_status.agents and run_agent_log still return the completed agent record after a restart on the same data dir', async () => {
    ...
    try {
      ...
      for (let i = 0; i < 100 && status.status === 'running'; i++) {
        ...
        status = await facade1.runStatus({ runId }, AUTH_DISABLED, false, null);
      }
      ...
      const logAfterRestart = await facade2.runAgentLog({ runId, agentId: 'agent-1' }, AUTH_DISABLED, false, null);
