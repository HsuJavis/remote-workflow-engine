// IT-171 (v24 Gate 7.5 defects D-6, D-7, D-13 — adjudication #5 E-7 "fix this round"): three
...
beforeAll(async () => {
  tmpDir = mkdtempSync(join(tmpdir(), 'rwe-it171-'));
...
describe('workflow_describe advertises the arguments it accepts (IT-171, D-7, REQ-118)', () => {
  ...
  it("`version` is the ONLY way to see a never-published workflow at all — without it the call is refused, which is why advertising it matters", async () => {
    const name = 'it171-never-published';
...
describe('workspace_push advertises only codes it can answer (IT-171, D-6, REQ-118)', () => {
  ...
  it("a kind:'hook' push answers the code that IS advertised", async () => {
    const r = await call('workspace_push', { workflow: WF, kind: 'hook', name: 'it171-hook', files: [] });
...
describe('a global asset lists as builtin (IT-171, D-13, REQ-113)', () => {
  it("workspace_list marks the engine-level tree's rows builtin:true, workflow-owned ones false", async () => {
    ...
    expect(rows.find((r) => r.name === 'it171-global')).toMatchObject({ scope: 'global', builtin: true });
    expect(rows.find((r) => r.name === 'it171-own')).toMatchObject({ scope: 'workflow', builtin: false });
  });
});
