...
// real workflow_agent_log + HTTP endpoint. Gateway faked only for structure assertions.
//
// CI-safe assertion (this test, real:false):
//   - workflow_agent_log response carries `hasMore:boolean` at the top level; NOT the current
//     ResultEnvelope<TranscriptEvent[]> wrapping (which has no hasMore field)
//   - AgentRecord.state in workflow_status is canonical (queued|running|done|failed)
//
...
//   - Real agent execution: workflow_agent_log carries harness.prompt + harness.tools + harness.model
//
...
//
// Red reason (CI-testable): workflow_agent_log currently returns ResultEnvelope<TranscriptEvent[]>
...
describe('VAL-082: harness detail in workflow_agent_log (REQ-073)', () => {
  it('workflow_agent_log response carries hasMore:boolean field (CI-safe, TASK-070 shape change)', async () => {
    ...
    const sub = await callTool('workflow_run', { script: 'return "val082";' }) as { runId?: string };
    ...
    const log = await callTool('workflow_agent_log', { runId, agentId: 'agent-1' }) as AgentLogEnvelope;
    // TASK-070: hasMore is a required field in the new response shape.
    expect('hasMore' in (log ?? {})).toBe(true);
  });

  it('AgentRecord.state in workflow_status is canonical — never idle/completed in JSON (CI-safe regression)', async () => {
    ...
    const sub = await callTool('workflow_run', { script: 'return "canonical-check";' }) as { runId?: string };
    const runId = sub?.runId!;
    await pollDone(runId, 8000);
    const status = await callTool('workflow_status', { runId }) as { agents?: Array<{ state?: string }> };
  ...
  it('harness field shows model, prompt, tools/skills, no secret (LLM-gated; headless DOM at Gate 7.5)', async () => {
    ...
    const sub = await callTool('workflow_run', {
    ...
    const log = await callTool('workflow_agent_log', { runId, agentId }) as AgentLogEnvelope;
