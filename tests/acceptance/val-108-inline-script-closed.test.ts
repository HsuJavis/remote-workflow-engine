...
// needed for the refusal path; `workflow_resume` uses a marker script with no agent() call.
//
// Red reason: `workflow_run`/`workflow_resume` accept inline `script` today, and `tools/list` still
...
describe('REQ-098: a hand-rolled inline-script call is refused, real HTTP (VAL-108)', () => {
  it('workflow_run({script}) over the real transport is refused INLINE_SCRIPT_CLOSED with the migration recipe', async () => {
    const r = await toolCall('workflow_run', { script: `return 'nope';` });
    expect((r['error'] as { code?: string } | undefined)?.code).toBe('INLINE_SCRIPT_CLOSED');
  });

  it('tools/list advertises no `script`/`scriptSha256` on workflow_run and no `script` on workflow_resume', async () => {
    const body = await rpc('tools/list', {});
    const result = body['result'] as { tools?: Array<{ name: string; inputSchema?: { properties?: Record<string, unknown> } }> } | undefined;
    const run = result?.tools?.find((t) => t.name === 'workflow_run');
    const resume = result?.tools?.find((t) => t.name === 'workflow_resume');
  ...
  it('a plain workflow_resume({runId}) with no script parameter is never refused INLINE_SCRIPT_CLOSED (unchanged plain-resume path)', async () => {
    ...
    const run = await toolCall('workflow_run', { name: 'val108-resume' });
    ...
    const resumed = await toolCall('workflow_resume', { runId });
