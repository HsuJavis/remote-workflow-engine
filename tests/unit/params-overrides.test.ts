...
// directly), plus the appendPrompt byte-only reporting rules and the UNKNOWN_ALIAS/PARAM_UNKNOWN
...
const contract = {
  agents: {
    ...
    review: {
      model: { type: 'enum', enum: ['sonnet-5', 'opus'], default: 'opus' },
...
describe('v24: validateUserOverrides — per-agent overrides (UT-147, DES-145)', () => {
  ...
  it.each(LOCKED_KEYS)('locked key "%s" ⇒ PARAM_LOCKED even when supplied directly to the pure function', (key) => {
    const result = validateUserOverrides(contract, { agents: { plan: { [key]: 'anything' } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('PARAM_LOCKED');
  ...
  it('a key that is neither locked nor tunable ⇒ PARAM_UNKNOWN', () => {
    const result = validateUserOverrides(contract, { agents: { plan: { nonsense: 1 } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
  ...
  it('a tunable key the agent does not declare (e.g. appendPrompt on "review") ⇒ PARAM_UNKNOWN', () => {
    const result = validateUserOverrides(contract, { agents: { review: { appendPrompt: 'hi' } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('PARAM_UNKNOWN');
  });

  it('model outside its declared enum ⇒ PARAM_OUT_OF_RANGE, not UNKNOWN_ALIAS', () => {
    const result = validateUserOverrides(contract, { agents: { plan: { model: 'gpt-5' } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('PARAM_OUT_OF_RANGE');
  });

  it('model with no declared enum and an unresolvable alias ⇒ UNKNOWN_ALIAS', () => {
    const noEnum = {
      agents: { plan: { ...contract.agents.plan, model: { type: 'string', default: 'sonnet-5' } } },
      args: {},
    } as unknown as import('../../src/params/contract.js').ParamContract;
    const result = validateUserOverrides(noEnum, { agents: { plan: { model: 'ghost-model' } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('UNKNOWN_ALIAS');
  });

  it('appendPrompt over the byte ceiling ⇒ PARAM_OUT_OF_RANGE, content never echoed in detail', () => {
    const result = validateUserOverrides(contract, { agents: { plan: { appendPrompt: 'x'.repeat(500) } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('PARAM_OUT_OF_RANGE');
  ...
  it('appendPrompt within bounds tunes cleanly', () => {
    const result = validateUserOverrides(contract, { agents: { plan: { appendPrompt: 'short note' } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(true);
  ...
  it('two agents tuned in one call, each resolved against its own spec', () => {
    const result = validateUserOverrides(
      contract,
      { agents: { plan: { effort: 'high' }, review: { model: 'sonnet-5' } } },
      new Set(['sonnet-5', 'opus']),
      DEFAULT_CEILINGS,
    );
    expect(result.ok).toBe(true);
    expect((result as { value: { agents: Record<string, unknown> } }).value.agents).toEqual({
      plan: { effort: 'high' },
      review: { model: 'sonnet-5' },
    });
  });
  ...
  it('raw undefined ⇒ ok with agents:{} (no overrides supplied at all)', () => {
    const result = validateUserOverrides(contract, undefined, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result).toEqual({ ok: true, value: { agents: {} } });
  });

  it('timeoutMs below the declared minimum ⇒ PARAM_OUT_OF_RANGE', () => {
    const result = validateUserOverrides(contract, { agents: { plan: { timeoutMs: 100 } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('PARAM_OUT_OF_RANGE');
  });

  it('effort outside its declared enum ⇒ PARAM_OUT_OF_RANGE', () => {
    const result = validateUserOverrides(contract, { agents: { plan: { effort: 'xhigh' } } }, new Set(['sonnet-5']), DEFAULT_CEILINGS);
    expect(result.ok).toBe(false);
    expect((result as { code?: string }).code).toBe('PARAM_OUT_OF_RANGE');
  });
});
