...
describe('InMemoryRunStore', () => {
  ...
  it('listRuns returns all created runs', async () => {
    const store = new InMemoryRunStore(CLOCK);
    await store.createRun({ origin: 'local', script: 'return 1;', name: 'wf-a' });
    await store.createRun({ origin: 'local', script: 'return 2;', name: 'wf-b' });
  ...
  it('getEffectiveParams returns the effectiveParams snapshot passed to createRun', async () => {
    ...
    const runId = await store.createRun({ origin: 'local', script: 'return 1;' }, 'v1', snapshot);
