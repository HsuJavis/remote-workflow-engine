...
>
> 目前部署狀態（v20，2026-08-19）：systemd user service `rwe.service`，綁定
...
> **v16 安全修補**：`/authorize` 在寫入 oauth_state 前驗證 `redirect_uri` 必須為 loopback URI（RFC 8252）；
> 過期 auth 表列由 GC sweep 自動清除（`gcExpired()` 已接入 sweep，`workspaceTtlMs` 正確轉發至 GC 間隔）。
> **v17 DCR（RFC 7591）**：`GET /.well-known/oauth-authorization-server` 新增 `registration_endpoint`；
> `POST /register` 實作 RFC 7591 Dynamic Client Registration，讓 Claude Code 等 MCP 用戶端可零設定
...
> **v20 refresh tokens + callback success page**：AS metadata 新增 `grant_types_supported:["authorization_code","refresh_token"]`、
> `scopes_supported:["openid","email","offline_access"]`、`token_endpoint_auth_methods_supported:["none"]`、
> `authorization_response_iss_parameter_supported:true`；`/authorize?...&scope=...%20offline_access` 流程在 `/token`
> 回傳 `refresh_token`（opaque、sha256-at-rest、~90 天 TTL）+`expires_in`（always）+`scope`（always echoed）；
> `POST /token grant_type=refresh_token` 核發新 `access_token` + 輪換 `refresh_token`（單次使用、RFC 9700）；
> 已消費 token 重播 → `400 invalid_grant`；無 `offline_access` 則不核發 `refresh_token`；
> 新增第 5 張 auth SQLite 表 `refresh_tokens`（`oauth_state.scope` + `auth_codes.scope` 欄位，idempotent ALTER）；
> `gcExpired()` 清除過期 refresh_tokens。`/oauth/google/callback` 改為回傳 200 HTML 成功頁（而非裸 302）：
> `id="callback-url"` 含原始 redirect URL（供 headless 貼上）+ meta-refresh/JS 自動轉跳（供同機器自動捕捉）。
> Gate 7.5 v20 **PASSED**：VAL-095 real:true pass（8 live curl checks）；IT-078 37/37（incl cases 23-28 refresh rotation）；
> val-095 9/9；val-096 5/5；val-097 8/8；1369/1369 pass（233 files）；生產服務重啟確認（refresh_tokens table、scope cols 存在；37 tools）；
> trace −check exit 0（0 未驗證，0 僅mock）。完整驗證證據見 `.sdlc/features/001-remote-workflow-engine/08-validation.md`。


## §0 Quickstart — 開機序列（可逐字貼上執行）

> 以下指令與 README quickstart 一致，是 v20 validator 實際跑過的步驟，本輪零文件缺口（無新設定鍵）。
...
| 2026-08-19 | v20 | **refresh tokens + callback success page**（REQ-012 v20）Gate 7.5 PASSED: AS metadata 新增 `grant_types_supported:["authorization_code","refresh_token"]`、`scopes_supported:["openid","email","offline_access"]`、`token_endpoint_auth_methods_supported:["none"]`、`authorization_response_iss_parameter_supported:true`；`/authorize?scope=...%20offline_access` + `/token` 回傳 `refresh_token`（opaque、sha256-at-rest、~90 天 TTL）+ `expires_in`（always）+ `scope`（always echoed）；`POST /token grant_type=refresh_token` 輪換 `refresh_token`（RFC 9700 rotation，單次使用；已消費 token → `400 invalid_grant`）；無 `offline_access` → 不核發 `refresh_token`；`/oauth/google/callback` 改為 200 HTML 成功頁（`id="callback-url"` 含原始 URL 供 headless 貼上 + meta-refresh/JS 自動轉跳）。新第 5 張 auth 表 `refresh_tokens`（`oauth_state.scope`/`auth_codes.scope` idempotent ALTER）；`gcExpired()` 清除過期 refresh_tokens。live curl 8 checks 全通（AS metadata、callback 200 HTML、offline_access 流程、refresh rotation、replay 400、無 offline_access 無 refresh_token、refreshed bearer /mcp 200、across-expiry 401→refresh→200 37 tools）。VAL-095 9/9；IT-078 37/37（+5 v20 cases 23-28）；val-096 5/5；val-097 8/8；1369/1369（233 files）；生產服務重啟確認（5 auth 表、scope cols 存在；37 tools；workflow_run completed）；trace −check exit 0（0 嚴重缺口，0 未驗證，0 僅mock）。 | 無破壞性變更；**無新設定鍵**（`REFRESH_TTL_MS` 為程式碼常數，非設定鍵）；**自動遷移**：TokenStore 建構子在首次啟動時執行 `ALTER TABLE oauth_state ADD COLUMN scope TEXT`、`ALTER TABLE auth_codes ADD COLUMN scope TEXT`（idempotent），並建立 `refresh_tokens` 表（`CREATE TABLE IF NOT EXISTS`）；無須手動操作；無新工具、無新 port |
