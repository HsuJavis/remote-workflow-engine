...
## v35 — REQ-205..210: 失敗原因落地、args 不再是 null、agent 全垮要說出口、掃描器不再把字串當程式碼 (TASK-231..238)

Eight tasks, and the split is **file ownership**, not taste: ~20 implementers share one working
tree, so a file touched by two v35 ARCH rows must belong to exactly ONE card. That single rule
produces this shape — `src/run-manager.ts` (ARCH-141 caller / 142 / 144 / 146) is one card even
though it closes three REQs; `src/server.ts` + `src/mcp-facade.ts` (ARCH-147 forwarding, ARCH-152
`initialize`, and two of ARCH-149's four read-caller markers) are one card even though the work is
two kinds. Landing order: **231 → 232 → 233 → 234 → 235 → 236 → 237 → 238**. Only three edges are
hard: TASK-231 before TASK-236 (`materializeArgDefaults` is imported), TASK-235 before TASK-236
(the port method must exist before its caller), TASK-233 before TASK-237 (the `unscannable` field
...
- **des:** DES-235
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/params-contract.test.ts tests/unit/params-resolve.test.ts tests/unit/workflow-describe-projection.test.ts` → green on: (1) `materializeArgDefaults({}, {url:{type:'string',default:'https://x'}})` returns `{url:'https://x'}` and `materializeArgDefaults({url:'y'}, …)` returns `{url:'y'}` (caller wins); an unknown caller key passes through untouched; a spec with no `default` key contributes nothing (a declared `default: undefined` is NOT a default); the function is pure (input objects not mutated); (2) registering `args:{n:{type:'number',default:'abc'}}` is now refused at registration by the **existing** `checkValueAgainstSpec` path (not by a new validator), while `default: 7` is accepted — `contract.ts:428-431`'s `'args spec cannot declare a default (never applied)'` string is gone from `src/`; (3) `RunParams` carries `args?: Record<string, unknown>` and `EffectiveCallParams` is `Omit<RunParams,'provenance'|'args'>`, so `workflow_describe`'s `params.agents.<label>` projection gains NO `args` key (the Gate-2 constraint 2 leak) — asserted, not assumed. No file outside `files:` is touched.
- **estimate:** S
- **iter:** v35
...
- **des:** DES-236
- **dod:** `npm ls acorn --omit=dev && npx tsc --noEmit && npx vitest run tests/unit/script-spans.test.ts` → green on: (1) `acorn` resolves with `--omit=dev` and `package.json`'s `dependencies.acorn` is an **exact** version (no `^`/`~`); (2) `nonCodeSpans` returns `{ok:true}` with spans covering a string literal, a template quasi (and NOT the `${…}` expression between quasis), a regex literal (`const re = /["']/;`) and both comment forms, all as half-open `[start,end)` offsets **in the input's own coordinate system**; (3) a sloppy-mode-only body (`var let = 1;` / legacy octal `010`) parses `{ok:true}` — the `sourceType:'script'` pin, and the case that fails under the first-draft module mode; (4) an unparseable body returns `{ok:false, reason}` and **never** a partial span set. No file outside `files:` is touched.
- **estimate:** S
- **iter:** v35
...
- **des:** DES-237
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/workflow-meta-scan.test.ts tests/integration/register-scan-spans.test.ts tests/unit/workflow-meta.test.ts tests/unit/skeleton-graph.test.ts` → green on: (1) the six REQ-208 cases register clean through the REAL `workflow_register` (regex literal, `${ await agent(…) }` substitution, escaped quote, comment apostrophe, prose `agent (mode A)` inside a prompt string, sloppy-mode-only script); (2) every pre-existing `SCAN_VIOLATION` test stays green **unchanged** and a genuine unlabeled `agent(` after a literal-heavy prelude still reports its own real `line`; (3) `AgentCallScan.index` for a real call is byte-identical to the pre-change value (the DES-174 skeleton↔scan join key — one fixture asserting the number, not that "a number exists"); (4) an oracle failure yields exactly one violation `{code:'SCRIPT_UNSCANNABLE', line:1}` → the existing `SCAN_VIOLATION` refusal from `workflow-catalog.ts`, **and** `scan.unscannable === true` is readable by a consumer that cannot refuse — `src/dashboard.ts:301`'s render marks the graph unscannable instead of drawing an empty one; (5) a `checkMeta` result with no `span` does NOT invoke the oracle (today's refusal wording survives — ordering, not code). `src/server.ts:540` and `src/mcp-facade.ts:494` are the other two read callers and are TASK-237's files on purpose — do NOT edit them here. No file outside `files:` is touched.
- **estimate:** M
- **iter:** v35
...
- **files:** src/check-mermaid.ts, tests/unit/check-mermaid.test.ts
- **des:** DES-238
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/check-mermaid.test.ts tests/unit/check-mermaid-v2.test.ts tests/integration/describe-mermaid-roundtrip.test.ts` → green on: (1) `checkMermaid`'s 5th parameter has no `?` and `check-mermaid.ts:285-300`'s `if (v2)` guard is gone — a 4-argument call is a **compile** error (asserted by `tsc`, plus one `@ts-expect-error` line in the test file so the guarantee has an executable statement); (2) all ~15 converted call sites in `check-mermaid.test.ts` pass a real `expected` graph, and **at least one converted case asserts a v2 rule FIRING** (`DIAGRAM_DIRECTION`/`LANE_MISMATCH`/`TOOLS_MISMATCH`/`EDGE_MISMATCH` returned for a diagram that is v1-clean) — the R6 trap: 15 rubber-stamped expectation objects are the false green wearing a required parameter as a hat; (3) `workflow-catalog.ts:548`, the one production caller, is untouched (it already passes five). No file outside `files:` is touched.
- **estimate:** M
- **iter:** v35
...
- **des:** DES-231
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/run-store-error.test.ts tests/integration/run-error-read-sites.test.ts tests/unit/run-store.test.ts tests/unit/sqlite-run-store-usage-projection.test.ts tests/integration/run-list.test.ts tests/integration/usage-projection-gap.test.ts` → green on: (1) `RunStore` declares `recordError`/`getError` and **both** implementations satisfy them (`SqliteRunStore` and `InMemoryRunStore` — the in-memory one is what lets the redaction/bound tests run with no SQLite file); (2) `recordError` writes `UPDATE runs SET error=?` **first** and appends the `{type:'error',…}` journal line **second** (same order as `recordResult`), and one call leaves the column and the journal line carrying the **byte-identical** value — the "ONE method, two surfaces, cannot drift" claim, asserted; (3) ALL FOUR read sites carry the field for one failed run in one test: `listRuns()`, `list({})`, `getRun()` (→ `RunStatusView.error`) and `getError()`; (4) `error` is surfaced **only when `status === 'failed'`** — a row whose `error` column is non-NULL but whose status is `interrupted`/`completed` (the crash-window row) reports no error on any read surface; (5) `failedAgentCount` from `_USAGE_PROJECTION` is emitted under `row.agentCount != null && row.agentCount > 0` — **omitted, never `0`**, for a run with no snapshot row (every non-terminal run), for a `{usage}`-only `backfillUsage` row, and for a zero-agent run; present and correct for a terminal mixed pass/fail run; (6) a pre-v35 row (`ALTER TABLE` applied to an existing db file, `error` NULL) reads back cleanly on all four sites. No file outside `files:` is touched.
- **estimate:** M
- **iter:** v35
...
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/errors-to-err.test.ts tests/unit/run-manager-error-capture.test.ts tests/integration/run-error-restart.test.ts tests/integration/run-args-resume.test.ts tests/integration/run-health-count.test.ts tests/integration/redact-sweep.test.ts tests/integration/resume-legacy-params.test.ts` → green on: (1) `toErr` lives in `src/errors.ts`, is exported, pure and **unbounded**, and `run-manager.ts`'s **two** call sites (`:1225` and the nested-workflow outcome at `:1306`) both import it; (2) `capErrorEnvelope` is applied at the ONE persist site **after** `redact()` — an oversize `message` comes back truncated with a visible marker, and a secret **straddling the cut** comes back as a redaction marker, never as two halves (the R-G9 ordering, asserted by a test that fails if the composition is rebuilt the other way round); (3) a failed run's reason is redacted at capture with a **real injected `_secretValueProvider`** and a real seeded secret — the marker is PRESENT in `runs.error` and in `journal.jsonl` and the raw value absent from both (an absence-only assertion passes vacuously); (4) ordering: a store spy records `recordError` strictly before `recordTransition(…,'failed')`; if `recordError` **throws**, the run still transitions to `failed` (`try/finally`) and the continuation's `.catch` leaves no unhandled rejection; (5) restart honesty — a SECOND `RunManager` over the same SQLite file answers `run_result` with the stored `{code,message}`, and a **pre-v35** `failed` row with `error IS NULL` answers `RUN_FAILED` / "no reason was recorded", neither answering `RUN_NOT_TERMINAL`; a run that is `interrupted` with a non-NULL error column still answers `RUN_NOT_TERMINAL` (the gated read); (6) args — a bare `run_start` gives the script `{}`; a declared `args.url.default` reaches the script AND `runs.effective_params.args`; **suspend → resume** and **restart-rehydrate → resume** both hand the script the same args as the first dispatch; a planted legacy `args='null'` row resumes to `{}`; (7) a run whose `args` carried a provisioned secret value **still resumes** (the marker scan is scoped to the dispatched fields) and the script receives the raw value; (8) `failedAgentCount` folds from `_mergeLive`'s agents on the live path AND from the restored snapshot after restart, is omitted for a zero-agent run, and agrees with the SQL twin per DES-234's five-case matrix. No file outside `files:` is touched.
- **estimate:** L
- **iter:** v35
...
- **des:** DES-239
- **dod:** `npm run gen:authoring && npx tsc --noEmit && npx vitest run tests/unit/workflow-describe-projection.test.ts tests/unit/tool-specs.test.ts tests/unit/authoring-guide.test.ts tests/unit/authoring-md-generated.test.ts tests/unit/tool-schema-drift.test.ts tests/unit/compose-config-v2-wiring.test.ts tests/integration/initialize-instructions.test.ts tests/integration/guide-examples-register.test.ts` → green on: (1) `workflow_describe`'s `params.agents.<label>.timeoutMs` carries `attempts` and `worstCaseMs === timeoutMs * attempts`, with `attempts = 1 + max(0, retries)` read from the **effective deployed** config forwarded at the composition root (`server.ts:825` → `McpFacadeDeps` → `projectWorkflowDescribe`), and `compose-config-v2-wiring.test.ts`'s sweep covers the new forward — a config `retries: 3` changes the advertised number (the `composeConfig` wiring class, twice bitten); (2) both `initialize` results (`server.ts:1240` and `:1482`) carry `instructions` containing `ENVELOPE_NOTE` **and** a guide size **equal to a value the test computes itself** from the same tool-result object a caller receives (no regex for `/\d+ ?KB/`, no module-scope memo — the number is computed per `initialize`); (3) `run_start.args`' description states `{}`-on-omission and where a default comes from; `run_result`/`run_status`/`run_list` descriptions state `error:{code,message}` on a failed run and `failedAgentCount`'s **omission semantics** (absent ≠ healthy; `run_list`'s count is terminal-only — poll `run_status` for a live one); `ENVELOPE_NOTE` is one exported constant consumed by the handshake, not two wordings; (4) the guide teaches the three facts — a failed sequential `await agent()` returns `null` and does not throw (with the `if (out === null)` self-protection pattern), `timeoutMs` bounds ONE attempt and the deployed retry count multiplies it (stated WITHOUT the SDK-only untimed exception — the two gateways disagree there, `client.ts:515` vs `claude-agent-sdk-client.ts:507`), and every tool result arrives as a JSON string inside `content[0].text`; (5) `docs/AUTHORING.md` byte-equal to the regenerated builder output in this same commit; (6) the doc-example guard registers every script+mermaid pair extracted from `docs/AUTHORING.md`, `DEPLOY.md` and `README.md` through the **real** `workflow_register` (DES-239's extraction rule), and fails when a file yields fewer pairs than its declared minimum; (7) the two remaining `SCRIPT_UNSCANNABLE` read callers — `server.ts:540` (skeleton graph) and `mcp-facade.ts:494` (per-call projection) — surface `scan.unscannable` as a visible marker instead of an empty result (TASK-233 owns the field and the other two callers). No file outside `files:` is touched.
- **estimate:** L
- **iter:** v35
...
- **des:** DES-240
- **dod:** `RWE_REQUIRE_BROWSER=1 npx vitest run tests/unit/dashboard-lib-strings.test.js tests/acceptance/val-232-run-error.test.ts` → green on: (1) against a REAL failed run on a real booted engine, the run **detail** view renders `error.code` and `error.message` next to the terminal status it already renders, and the run **list** (`workflow.js`'s `renderChipsAndTable`, :242) shows the code on the `failed` row — both reading the fields `/api/runs` and `/api/runs/:id` already carry (no new endpoint, no new fetch, no client-side computation); (2) a `completed` run and a pre-v35 `failed` run with no recorded reason render the status with **no** error block and no `undefined` text; (3) both new strings exist in zh-TW and en through `src/dashboard/lib/strings.js` (no literal in a view file). `failedAgentCount` is deliberately NOT rendered — see DES-240's note. No file outside `files:` is touched.
- **estimate:** S
- **iter:** v35

## v36 — REQ-211..216: 單版本刪除、稽核身分、營運日誌、控制檔、拒絕標記穿越 sandbox、K1–K8 結清 (TASK-239..247)

Nine cards. v35 split on **file ownership** so cards could float; v36 cannot — the architecture's
Constraint 1 is a real dependency chain and three cards legitimately touch `src/run-manager.ts`
(K1/K2's capture sites, REQ-213's `_transition` line, REQ-215's ledger). So this round splits on
**runtime coupling** (the panel's converged rule: a seam whose halves have no independently testable
intermediate state is ONE card) and pays for it with an explicit landing order. The Gate 6
dispatcher batches on `files:` — two cards sharing a `files:` entry must never land in one wave;
where that matters the card says which card it lands after.

**Landing order: 239 → 240 → 241 → 242 → 243 → 244 → 245 → 246 → 247.** Five hard edges:
TASK-239 first (it is the security fix, and TASK-246's ledger imports its function); **TASK-240
before TASK-241** (Constraint 1: REQ-213 makes the log stream load-bearing, and two instances
interleaving into one `.rwe.log` would birth the new observability already corrupt); TASK-241 before
every emitter (242/243/244) — ARCH-155 `deps: ARCH-159`, and shipping the constructor plumbing with
the module is what stops REQ-211's one destructive action from landing in a window where it logs
nothing; TASK-242 before TASK-243/244 (both emit `actor`, which TASK-242 mints); TASK-246 before
TASK-247 (the guide sentence describes the marker TASK-246 creates).

**The composition-root event IT grows with each call site, in the same commit that adds it** — 242
writes it with `catalog.register`+`catalog.publish`, 243 adds `run.terminal`, 244 adds
`catalog.deregister`. That replaces the sweep-row probe ARCH-159/171 named: `eventSink` and the
derived `attempts` are not `FileConfig` keys (`main.ts:84-96` is the closed list), so a row in
`compose-config-v2-wiring.test.ts` would assert a key nobody sets — see DES-243/DES-249.

**ARCH-172 carries a pending `owner_decision` (`listSummaries()` pagination) and it does NOT block
this list.** REQ-216/K5's actual deliverable is the ruling, and ADR-079 made it (no LIMIT on
`listRuns()`); TASK-245 writes that ruling into the port contract. No card assumes either branch of
...
- **des:** DES-241
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/errors-capture-failure.test.ts tests/unit/run-manager-seedref-redaction.test.ts tests/unit/errors-to-err.test.ts tests/unit/run-manager-error-capture.test.ts tests/integration/redact-sweep.test.ts` → green on: (1) `captureFailure(err, secrets, maxBytes = MAX_ERROR_ENVELOPE_BYTES)` is exported from `src/errors.ts`, pure (no clock/store/manager), and is exactly `capErrorEnvelope(redact(toErr(err), secrets), maxBytes)` in that order; `capErrorEnvelope` gains the defaulted second parameter and every existing caller's behaviour is byte-identical; (2) **K2**: `seedRefView.failDetail` is `captureFailure(err, this._secretValueProvider?.entries() ?? [], 200).message` — with a REAL injected `SecretValueProvider` whose value appears in the fetch error message, `failDetail` contains `‹secret:NAME›` and **not** the raw value, on a message long enough that the 200-byte bound actually cuts (an absence-only assertion passes vacuously); (3) **B6**: `failCode` keeps its three-value domain — a `TypeError` off the fetch path still yields `failCode:'SEEDREF_FETCH_FAILED'`, never `'TypeError'`, and `seedRefFail.code`'s own computation (`:686`) is untouched; (4) **K3** byte-exact: at `maxBytes=200` an over-bound message whose cut opens a `‹secret:NAME›` with a 256-code-unit name comes back ≤ `200 + (MARKER_PREFIX.length + MAX_SECRET_NAME_CHARS + 1) * 3` bytes (= 995) with the marker INTACT, and the same arithmetic at 4096 (= 4891); both numbers computed in the test from the exported constants, never typed as literals; (5) **K1**: `run-manager.ts`'s two pre-existing capture sites (`:764-766` seedRefFail→`resultError`, `:1285-1288` dispatch outcome) are two-line `captureFailure(...)` callers and no inline `redact(toErr(...))` composition remains in `src/run-manager.ts` (grep assertion). **Three commits, this order: (a) the pure function + its tests, no call-site change; (b) K2's `failDetail`; (c) K1's collapse** — the security fix is reviewable as its own commit without a refactor wrapped around it, which is ARCH-170's intent; its `api:` line (`.message`) wins over its prose, because the prose reading regresses `failCode`. No file outside `files:` is touched.
- **estimate:** M
- **iter:** v36
...
- **des:** DES-242
- **dod:** `npx vitest run tests/integration/deploy-control-files.test.ts` → green on: (1) the `RWE_CONFIG_PATH` export and the `RWE_INSTANCE`/`RWE_PID_FILE`/`RWE_LOG_FILE` derivation move **above step 1**, and `--dry-run` is handled immediately after — asserted by the dry run completing without `npm install` running (no `步驟 1/5` line on stdout, exit 0); (2) two `RWE_CONFIG_PATH` values **in one directory** (`rwe.config.json`, `scratch.config.json`) yield two distinct pid paths and two distinct log paths, each `.rwe.<config-basename>.{pid,log}` beside its own config — the collision case; a two-directory test would pass on the broken `dirname`-only design; (3) the log is created by `(umask 077; touch "$RWE_LOG_FILE")` in a **subshell** followed by `chmod 600` — mode is `0600`, and `grep` proves no bare top-level `umask` line exists (a bare one is inherited by the engine and turns every SQLite db, workspace and CAS blob 0600); (4) a line pre-seeded into the log **survives** a second `--dry-run`/start — the start path appends with `>>` and no `>`/`: >` truncation of `$RWE_LOG_FILE` remains in the script (ADR-076 makes this file the only audit record that exists, so a restart must not erase the incident it is being read for); (5) `.gitignore` covers `.rwe.*.pid` and `.rwe.*.log` and keeps the two bare literals for existing trees; (6) `DEPLOY.md`'s v34 workaround paragraph states the new behaviour, prints `kill $(cat .rwe.<instance>.pid)`, and carries ADR-078's two disclosures — the log is PII-bearing after REQ-212, and the engine builds no rotation, so point your rotation tool at it. `--dry-run` writes **no** pid file and starts nothing. No file outside `files:` is touched.
- **estimate:** M
- **iter:** v36
...
- **des:** DES-244
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/catalog-actor.test.ts tests/integration/main-composition-root-events.test.ts tests/unit/authz.test.ts tests/integration/authz-enforcement-live.test.ts tests/integration/authz-owner-lookup.test.ts tests/unit/facade-refusal-arms.test.ts` → green on: (1) `canMutate(owner, actor)` is exported and pure, written `!owner || actor.bypass || owner === actor.id` — the **truthiness** form, because the legacy gate tests `row.owner` for truthiness (`:575,:669,:789`) and `owner === ''` is reachable on a pre-v15 row; (2) the table test: for every `(Principal.kind × args.principal present/absent × gate × owner ∈ {null,'','alice'})`, `canMutate(owner, actorFor(p,a,gate))` equals the legacy expression `!(owner && legacyId !== null && owner !== legacyId)` evaluated on the same inputs — this is the executable form of Gate-2 Constraint 10; (3) `actorFor(p, a, gate)` mints per **call site**: `register`/`insertVersion` pass `'attribution'` and `deregister`/`publish` pass `'bypass'`, so an admin registering a new version over **someone else's** name is STILL refused `NOT_WORKFLOW_OWNER` (R-1 — one named case, because the failure mode of getting this wrong is silence, not a red test); (4) `Actor` has exactly three fields `{id, bypass, idSource}` — no `kind`; `id` is always the attribution rule's answer, so an admin's own id is recorded **while** bypassing; an `auth-disabled` caller's `args.principal` is `idSource:'claimed'`, never `'authenticated'`; (5) `insertVersion`'s success path and `publish` emit `catalog.register`/`catalog.publish` through `this._eventSink` with `actor:{id,bypass,idSource}`, and `` console.log(`catalog.publish: …`) `` is gone from `src/` (grep assertion); (6) the composition-root IT boots the real root with an injected `write`, runs one real `workflow_register` + one real `workflow_publish`, and asserts the two JSON lines, their kinds and the full actor triple; (7) **every existing ownership test stays green unchanged** — if one needs its expectation edited, the refactor changed semantics and is wrong. No file outside `files:` is touched.
- **estimate:** M
- **iter:** v36
...
- **des:** DES-245
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/run-store-getspec-principal.test.ts tests/integration/run-terminal-event.test.ts tests/integration/main-composition-root-events.test.ts tests/unit/run-store.test.ts tests/integration/run-error-restart.test.ts` → green on: (1) `RunEntry.principal?: string`, assigned in `_newEntry` from `spec.principal` **and** on the rehydrate path (`_requireLive()`) from `getSpec()`; (2) `SqliteRunStore.getSpec()`'s SELECT includes the `principal` column (written at `:115-116`, read by the status view at `:305`, absent only here) and returns it on the `RunSpec`; a **both-stores agreement test** round-trips one run started with a principal through `SqliteRunStore` and `InMemoryRunStore` and gets the same `spec.principal` out — without this the resumed-run identity test passes on the in-memory twin while production emits `null`; (3) `_transition` emits `{kind:'run.terminal', runId, name, version:`v${entry.scriptVersion}`, outcome, principal}` when `to` is terminal — **`completed` included**, which is why the line is not inside a failure-capture function; `version` is emitted as the same `v<N>` **string** the catalog lines carry, so the two line kinds join; (4) a **resumed** run (a second `RunManager` over the same SQLite file) emits the same `principal` as its first dispatch — the R-2 case, and the half an incident investigation reads; (5) `run_start` refuses a caller-supplied `principal` argument (`additionalProperties:false`, `mcp-facade.ts:613`) — the guard that keeps `run.terminal`'s omission of `bypass`/`idSource` honest: if run admission ever starts honouring `args.principal`, this test goes red and forces the line to grow the field; (6) the composition-root IT gains its `run.terminal` assertion **in this commit**. Lands after TASK-242 (the IT file is that card's). No file outside `files:` is touched.
- **estimate:** M
- **iter:** v36

### TASK-244 — `deregisterVersion`: six outcomes in one pinned order, two DELETEs in one transaction, and the claim that must be released
- **status:** draft
...
- **des:** DES-246
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/catalog-deregister-version.test.ts tests/integration/deregister-version-pinned-run.test.ts tests/integration/main-composition-root-events.test.ts tests/unit/tool-schema-drift.test.ts tests/integration/deregister-clears-asset-tree.test.ts` → green on: (1) ~~**CORRECTION (v36, this gate — the implementer that landed the code already corrected the behaviour; only this DoD sentence was stale, matching DES-246's own "Gate-8 send-back (b) CORRECTION" note in its signature block):** `deregisterVersion(name, version, actor)` itself — the catalog method — answers its **five** own outcomes in the pinned refusal order **ownership → name-absent → version-not-found → channel → last-remaining** (ownership first so a stranger cannot enumerate versions by refusal type). `VERSION_PINNED_BY_RUN` is NOT a sixth outcome inside `deregisterVersion` at all: it is the FACADE's own probe (`mcp-facade.ts`'s `workflowDeregister`, `this.store.listRuns()`), and it runs FIRST, before the catalog mutation path is ever entered — DES-246's own boundary note states this plainly (a cross-database read that refuses must resolve before the mutation path is entered), so the true end-to-end pinned order for a version-scoped `workflow_deregister` call is **pinned-run (facade) → ownership → name-absent → version-not-found → channel → last-remaining (catalog)**, not "ownership → … → pinned-run" as this sentence read before the correction;~~ **[SUPERSEDED 2026-09-22, Gate 8 F6 send-back — struck-through text above kept per house style, do not implement against it]** `deregisterVersion(name, version, actor, pinnedRunId)` — the catalog method, now taking a **required 4th parameter** `pinnedRunId: string | null` — answers its own **six** outcomes in ARCH-155's pinned refusal order **ownership → name-absent → version-not-found → pinned-by-run → channel → last-remaining** (ownership first so a stranger cannot enumerate versions by refusal type). `VERSION_PINNED_BY_RUN` IS now a fourth outcome inside `deregisterVersion` itself, thrown from the `pinnedRunId` argument — the catalog neither computes nor verifies the fact, only refuses once it is non-null, at a ladder position AFTER ownership so the F6 defect (a non-owner learning a pinned-run fact before `NOT_WORKFLOW_OWNER` fires) cannot recur structurally. The FACADE still gathers the fact (`this.store.listRuns()`, ARCH-156 — the only module holding both database handles) but no longer refuses on it; it mints `actorFor(...)` once, above the probe, and passes both `actor` and `pinnedRunId` straight through. The true end-to-end order for a version-scoped `workflow_deregister` call is now **ownership → name-absent → version-not-found → pinned-by-run → channel → last-remaining**, all six inside one catalog method, matching ARCH-155/156 and ADR-075 as amended 2026-09-22 — not the struck-through sentence's "pinned-run (facade) → ownership → …", and not the send-back's own rejected one-line fix ("mint `actorFor` earlier and change nothing else", which is a no-op because `actorFor` is pure); `{removed:false, remaining:[]}` for an absent **name**, `VERSION_NOT_FOUND` for an absent **version**; (2) on success exactly **two** statements in one `.immediate()` transaction — `workflow_versions` and `workflow_diagrams` at `(name, version)` — with the `assets` rows and the `workflows` row read back **present** afterwards; (3) Gate-8 send-back CORRECTION: this item's premise was stale — `putDiagramResult`'s late-write guard has been `(name, version)`-keyed since it was written (v23, see TASK at :845's own dod: `SELECT 1 FROM workflow_versions WHERE name = ? AND version = ?`, and the `workflow_diagrams` insert's own `ON CONFLICT (name, version)`), never name-keyed, so there is no move to make here. What THIS card still owns: a diagram result that lands after its version was deleted writes nothing (the orphan-row bug that guard exists for) — already true unconditionally by inheriting the pre-existing `(name, version)` key, re-confirmed by IT-299's real defect fix (the diagramCache in mcp-facade.ts, a DIFFERENT in-memory cache, DID need a new invalidate() call on this card's version-scoped path — see IT-299); (4) the facade's optional `version` on `workflow_deregister` (schema + handler): absent ⇒ today's whole-name path, byte-identical, every existing code unchanged; (5) ~~the pinned-run probe normalizes **both** sides (`String(x).replace(/^v/,'')`) — a catalog row stored as `"3"` pinned by a genuinely non-terminal run (a real store fixture, not a mock) is refused `VERSION_PINNED_BY_RUN` naming the runId, and the catalog is never called;~~ **[SUPERSEDED 2026-09-22, F6]** the pinned-run probe normalizes **both** sides (`String(x).replace(/^v/,'')`) — a catalog row stored as `"3"` pinned by a genuinely non-terminal run (a real store fixture, not a mock) is refused `VERSION_PINNED_BY_RUN` naming the runId; the catalog **is** called (it owns the refusal now) but its two DELETEs never execute because the ladder throws first, at position 4, before reaching the transaction; (6) `TERMINAL` is imported from `run-store.ts`'s newly-exported `Set` — no sixth private copy; (7) **triggers**: `deregisterVersion` returns `claimedTriggers` = the ids declared **only** by the deleted version (`declaredTriggers` before minus after, computed inside the transaction) and the facade releases exactly those through its existing `:390-397` loop — asserted both ways: an id still declared by a surviving version is NOT released, an id only the deleted version declared IS; (8) both copies of `VERSION_CEILING_EXCEEDED`'s message (`:570`, `:614`) name the real call shape `workflow_deregister({name, version})`; (9) the `catalog.deregister` line carries `{name, version, actor}` and the composition-root IT gains its third assertion **in this commit**; (10) every whole-name `deregister` test stays green unchanged; (11) **new (F6)**: a discriminating UT proves ladder ORDER — a version both channel-published and run-pinned throws `VERSION_PINNED_BY_RUN` (position 4), never `VERSION_PINNED_BY_CHANNEL` (position 5); a non-owner (stranger) probing a run-pinned version is refused `NOT_WORKFLOW_OWNER` (position 1), never `VERSION_PINNED_BY_RUN`. No file outside `files:` is touched.
- **estimate:** L
- **iter:** v36
...
- **des:** DES-248
- **dod:** **RED-first: the real-child integration test is written and failing before any of the four files change** (a half-landed IPC field is dead code by construction — the false green this iteration exists to delete). `npx tsc --noEmit && npx vitest run tests/unit/sandbox-refusal-provenance.test.ts tests/integration/refusal-marker-real-child.test.ts tests/unit/sandbox-guards.test.ts tests/integration/run-error-restart.test.ts` → green on: (1) `markEngineRefusal(err, callSeq)` + a module-scope `WeakMap<object, number>` in `guards.ts`; `evaluateScript`'s `{kind:'error'}` carries `refusalRef` **read from the map, never off the error object**; (2) `ENGINE_REFUSAL_CODES` gains `'PARAM_UNKNOWN'` — an uncaught `PARAM_UNKNOWN` keeps its own code instead of flattening to `SCRIPT_ERROR`, **and** one raised inside `parallel()`/`pipeline()` now **propagates instead of returning `null`** (ADR-073, a deliberate and asserted script-semantics change); (3) four pure unit cases through the injected `SandboxApi`, no `fork()`: positive (a refusal marked `7`, rethrown by the script, surfaces `refusalRef:7`), negative (the script throws a **fresh** error → no ref), forgery (the script sets `e.refusalRef = 99` on the caught error → the map's `7` wins), `parallel()` identity (the ref survives the re-throw — Gate-2 Constraint 3's pin, so a future refactor that wraps the error there fails loudly); (4) the ledger on `RunEntry`: `refusals: Map<number, {code;message}>` filled via `captureFailure(err, secrets)` (TASK-239's function — the same redacted, bounded envelope every other failure surface gets), bounded at **8**, a 9th refusal increments `refusalsDropped` and **evicts nothing**; (5) refusals are recorded **only at the top-level wiring site** (`framePath === ''`), and one negative case pins the scope-out: a refusal raised inside a nested `workflow()` frame fails the run with its code and **no** marker (the nested boundary re-mints the error at `run-manager.ts:1375`, so the WeakMap key is gone by construction); (6) at settle, a `refusalRef` **this run's map contains** replaces the flattened envelope with the ledger entry; an **unknown** ref is dropped silently and today's path runs unchanged; (7) the 3-line drift test: the literal `ENGINE_REFUSAL_CODES` set in `guards.ts` (read by source-text regex — the duplication is forced by the child's `.js`→`.ts` value-import constraint and can never be refactored away) equals the recorded set `run-manager.ts` exports; (8) the ONE real-child IT (generous timeout, no matrix): a real script rethrowing a real engine refusal in a real forked child → `run_result.error.code` is the refusal code and the run-level envelope carries the ledger's structured entry, with the IPC **not** mocked. `host.ts` relays `typeof msg.refusalRef === 'number'` and validates nothing. Lands after TASK-239 and TASK-243 (both touch `run-manager.ts`). No file outside `files:` is touched.
- **estimate:** L
- **iter:** v36
...
- **des:** DES-249
- **dod:** `npm run gen:authoring && npx tsc --noEmit && npx vitest run tests/unit/gateway-attempts.test.ts tests/unit/authoring-guide.test.ts tests/unit/authoring-md-generated.test.ts tests/unit/tool-specs.test.ts tests/unit/tool-schema-drift.test.ts` → green on: (1) `attemptsFor(retries, timeoutMs)` is exported from the port module (`src/gateway/client.ts`, where `interface GatewayClient` is declared at `:178`) and is `timeoutMs === undefined ? 1 : 1 + Math.max(0, retries ?? 0)`, pinned by a table test over the four quadrants; (2) **both** conformers call the export — `claude-agent-sdk-client.ts:507` (already this rule) and `client.ts:515`, which today retries an **untimed** call and is therefore the deviant one; per-conformer assertion that the shared export is what produces the number (spy or source-text), **not** a behavioural matrix through both transports: that would mean mocking `fetch` **and** the Agent SDK to prove an import; (3) the guide gains the missing sentence — an `agent()` call with **no** `timeoutMs` runs **once**; retries apply only to a call that set one — beside ARCH-151's multiplied-worst-case text; (4) the **attestation boundary** in one sentence each on `workflow_authoring_guide` and the `run_result`/`run_status` tool descriptions: the run-level error's structured refusal marker is engine-attested (lifted only against the parent's own ledger), while `error.code` is **not** and never has been (`refusalCode()` matches `e.name`, `guards.ts:185`) — plus one negative case: a script forging `name:'PARAM_UNKNOWN'` produces the code and **no** marker; (5) `docs/AUTHORING.md` byte-equal to the regenerated builder output **in this same commit** (the byte-locked pair — a momentarily-red guide diff on a ~20-agent shared tree is ambiguous between 「known WIP」 and 「someone broke it」). Lands after TASK-246 ((4) describes the marker that card creates). No file outside `files:` is touched.
- **estimate:** M
- **iter:** v36

### TASK-248 — REQ-217/K5's owner ruling: paginate `listSummaries()`, add `RunStore.workflowMetrics()` for the two full-history averages, fix the order-dependent card picker it would otherwise have broken
- **status:** done
- **traces:** ARCH-172, ADR-080, REQ-217, REQ-216
...
- **des:** DES-250
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/run-manager-summarize-usage.test.ts tests/unit/workflow-metrics-store-agreement.test.ts tests/unit/home-view.test.ts tests/unit/workflow-metrics.test.ts tests/unit/dashboard-metrics.test.ts tests/integration/home-api.test.ts tests/integration/req217-pagination-full-history.test.ts tests/integration/dashboard-disclosure.test.ts` → green on: (1) `RunManager.listSummaries()` reads `store.list()` (limit 50 / hard cap 500, DES-152's existing numbers) instead of the unbounded `store.listRuns()` — the list-path cliff genuinely gone, not documentation-carried; (2) a new `RunStore.workflowMetrics(): Promise<Map<string | undefined, WorkflowMetrics>>` port method, implemented on `SqliteRunStore` as a single `GROUP BY name` SQL aggregate (`AVG()`/`COUNT()`/`SUM()`, never selecting `runs` into memory) and on `InMemoryRunStore` as a thin delegate to the EXISTING `computeWorkflowMetrics` pure fold (zero edits to that function or its own UT-073/UT-239 suite); a name with zero terminal runs is **absent** from the map on both, never a zero-valued row; `avgCostUSD` is `null` (never `0`) for a name whose terminal runs are all zero-agent/unpriced (REQ-186's "unmeasured is absent" applied to `AVG()` over zero priced rows); both-stores agreement pinned over one fixture seeded through the port only (createRun/recordTransition/saveSnapshot); (3) `RunManager.workflowMetrics()` thin-delegates to the store (no live-entry overlay needed — a run is only counted once terminal, and `_transition` always writes its snapshot); `server.ts`'s `/api/home` handler reads it instead of folding `computeWorkflowMetrics()` over the (now-paginated) `listSummaries()` output; (4) `buildHomeView`'s `activeRunId`/`latestRunId` picker compares `createdAt` directly instead of trusting positional "list order" — pinned by two new cases (ascending- and descending-order input resolve to the same winner) that the old "last one wins" loop would have failed once fed `list()`'s `DESC` order; (5) `dashboard-disclosure`'s golden key-set tests are unchanged (no field added/removed on `RunSummary`/`WorkflowMetrics`/`HomeView`) — only the computation source moved. Lands independently of TASK-239..247 (touches `run-manager.ts`/`run-store.ts` at different call sites; no textual overlap). No file outside `files:` is touched.
- **estimate:** M
- **iter:** v36

### TASK-249 — `RunStore.activeRuns()`: the RUNNING/`activeRunId` decision moves off `list()`'s page
- **status:** done
- **traces:** DES-251, DES-250, REQ-217
...
- **des:** DES-251
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/home-view.test.ts tests/unit/active-runs-store-agreement.test.ts tests/unit/workflow-metrics-store-agreement.test.ts tests/integration/req217-pagination-full-history.test.ts tests/integration/dashboard-disclosure.test.ts tests/acceptance/val-083-home-cards.test.ts tests/integration/home-api.test.ts` → green on: (1) a new `RunStore.activeRuns(): Promise<RunSummary[]>` port method (no LIMIT) returning every run whose status is in the exported `ACTIVE` set (`queued`/`running`/`suspended`/`interrupted` — the `RunStatus` union's non-`TERMINAL` complement), implemented on `SqliteRunStore` as a `WHERE r.status IN (...)` reuse of `list()`'s SELECT body and on `InMemoryRunStore` as a filter over the same `ACTIVE` set, both stores checked by a both-stores agreement test seeded through the port only (`createRun`/`recordTransition`), including the discriminating case — a single active run OLDER than a 55-row terminal flood is dropped by `list()`'s 50-row page but still returned by `activeRuns()`, on both stores; (2) `RunManager.activeRuns()` thin-delegates to the store (mirrors `workflowMetrics()` — no live-entry overlay needed, a non-terminal run's usage fields are irrelevant to this decision); `server.ts`'s `/api/home` handler fetches it alongside `catalog.list()`/`listSummaries()`/`workflowMetrics()`; (3) `buildHomeView` gains a REQUIRED 4th parameter, `activeRuns: RunSummary[]` — RUNNING/`activeRunId` resolve from it exclusively (never from the paginated `runs` page), `latestRunId`/`latestRunAt` stay sourced from `runs` alone (DES-250's already-accepted narrowing for those two fields, unchanged); pinned by a new UT-072 case — a workflow whose only active run is older than an (empty, for it) page still lands in RUNNING with `activeRunId` intact, and `latestRunId` stays `undefined` (asserted, not left as an accident); all 12 pre-existing UT-072 cases updated to pass their own `runs` array as the 4th argument too (semantics unchanged — `buildHomeView` still filters by `ACTIVE_STATUSES` internally); (4) `dashboard-disclosure`'s golden key-set tests are unchanged (no field added/removed on `RunSummary`/`HomeView`/`WorkflowCard`) — only `buildHomeView`'s internal source for one computation moved. Lands independently of TASK-248 (same files, disjoint code regions — the pagination/aggregate split TASK-248 shipped is untouched; only the active/latest-run picker loop, which TASK-248 itself last touched, is split into two loops over two different input arrays). No file outside `files:` is touched.
- **estimate:** S
- **iter:** v36

## v37 — REQ-218/REQ-219: the Bash jail the comments claimed, and the two modules whose green tests had no caller (ARCH-175..180 + the ARCH-107/016/025/017 amendments · ADR-082..085) — TASK-250..256

**Order.** TASK-250 (spike) is first and blocking — three DES rows are parameterized on its arms, and
both arms of each are already written down (DES-252/253/256/259), so no Gate-5 test is rewritten by
its outcome. TASK-251 → TASK-252/TASK-253 (both import the pure module). TASK-253 → TASK-255
(**rescue before delete**, never the reverse). TASK-254 and TASK-256 are independent of everything.
No two concurrently-runnable tasks share a `files:` entry.

### TASK-250 — the blocking spike: S1–S10 on the REMOTE host, one dated evidence file per arm
- **status:** done
- **traces:** ARCH-175, ARCH-025, ADR-082, ADR-083
- **files:** evidence/v37-spike/S1.md, evidence/v37-spike/S2.md, evidence/v37-spike/S3.md, evidence/v37-spike/S4.md, evidence/v37-spike/S5.md, evidence/v37-spike/S6.md, evidence/v37-spike/S7.md, evidence/v37-spike/S8.md, evidence/v37-spike/S9.md, evidence/v37-spike/S10.md
- **des:** — (no DES: this task returns measurements, not code; DES-252/253/256/259 each specify BOTH arms so the design is stable across every outcome)
- **dod:** `grep -l '^- \*\*verdict:\*\*' evidence/v37-spike/S*.md | wc -l` → **10**, each file carrying the date, the host, the exact command, its raw output and a one-line verdict. Arms: **S1** does `Options.sandbox.filesystem.allowWrite` actually confine a Bash write (arm 1)? **S2** if inert, does the same object via `Options.settings.sandbox` (arm 2)? **S3** else arm 3 (`additionalDirectories` + permission rules). **S4** does `PostToolUseFailure` fire for a sandbox-denied command (decides whether `agent.confinement_denied` is built at all — DES-256)? **S5** can a workspace-written `.claude/settings.json` weaken the sandbox? **S6** bubblewrap + unprivileged userns on the remote host, **plus the addendum**: which `claude` binary actually resolves (the one bundled in `@anthropic-ai/claude-agent-sdk` vs a PATH-installed one) and its version string. **S7** does a narrower `allowRead` override a broader `denyRead` — run **with `allowRead` set**, or it measures the wrong object (decides `DENY_READ_MODE`, DES-252). **S8** does `credentials.envVars[].mode:'mask'` hide `ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN`/`OPENROUTER_API_KEY` from the agent's shell **without breaking the CLI's own authentication** (ARCH-025 says a positive S8 joins the posture in THIS iteration — DES-252's conditional field)? **S9** does one real SDLC-shaped workflow still **complete** under the confinement — return the full `EACCES` inventory (`$TMPDIR`, `~/.npm/_logs`, `~/.cache`, `~/.config/git`, compiler scratch) as design input, plus S1's `bwrap` per-call setup latency. **S10** on a host where the sandbox cannot start, is `failIfUnavailable:true`'s failure **distinguishable** from an ordinary terminal failure — record the exact result shape (decides DES-259's detail mapping; if not distinguishable, ADR-083's revisit trigger is unsatisfiable from the log and that is recorded as a residual, not left silent).
- **estimate:** M
- **iter:** v37

### TASK-251 — the pure confinement module: `buildBashConfinement()` + `validateHostPathGrants()`, no `fs`, no `process`, no clock
- **status:** draft
- **traces:** ARCH-175, ARCH-016, ARCH-025, ADR-082
- **files:** src/gateway/bash-confinement.ts, tests/unit/bash-confinement.test.ts
- **des:** DES-252, DES-254
...
- **status:** draft
- **traces:** ARCH-177, ADR-084
- **files:** src/main.ts, tests/unit/compose-config-v2-wiring.test.ts, tests/unit/sandbox-config-wiring.test.ts, rwe.config.example.json, DEPLOY.md
- **des:** DES-253, DES-254, DES-255
...
### TASK-258 — `admissionRefusal()` at `RunManager.start()`: close the schedule/webhook admission routes DES-262's door does not cover (new row, ARCH-182, ADR-086, REQ-218)
- **status:** done
- **traces:** ARCH-182, DES-263, ADR-086, REQ-218
- **files:** src/types.ts, src/run-manager.ts, src/store/sqlite-run-store.ts, src/mcp-facade.ts, src/call-tool.ts, src/scheduler.ts, src/webhook-registry.ts, src/server.ts, tests/unit/admission-refusal.test.ts, tests/unit/run-manager-admission-order.test.ts, tests/unit/scheduler-remote-origin.test.ts, tests/unit/webhook-remote-origin.test.ts, tests/unit/sqlite-run-store-legacy-origin.test.ts
- **des:** DES-263
- **dod:** `npx tsc --noEmit && npx vitest run tests/unit/admission-refusal.test.ts tests/unit/run-manager-admission-order.test.ts tests/unit/scheduler-remote-origin.test.ts tests/unit/webhook-remote-origin.test.ts tests/unit/sqlite-run-store-legacy-origin.test.ts` → green on: (1) `admissionRefusal({posture,origin})` — pure, exported from `src/run-manager.ts` — returns `'CONFINEMENT_UNAVAILABLE'` iff `posture==='unconfined' && origin==='remote'`, `null` on every other combination including `posture===undefined`; (2) `RunManager.start()` calls it as the FIRST statement, ahead of `RUN_ADMISSION_LIMIT` and `INLINE_SCRIPT_CLOSED` — a remote+unconfined submission is refused `CONFINEMENT_UNAVAILABLE` even when the admission cap is already exhausted or the spec carries a closed inline script, proving order, not just outcome; (3) `RunSpec.origin: 'local'|'remote'` is REQUIRED — `tsc` finds every construction site; (4) `schedules.createdRemote`/`webhooks.createdRemote` (`INTEGER NOT NULL DEFAULT 0`) land via the existing idempotent `ALTER TABLE` idiom, written once at `schedule_create`/`webhook_create` from `ToolDeps.isRemoteSubmission`, and read back at `Scheduler.trigger()`, `server.ts`'s ticker dispatcher, and `WebhookRegistry.deliver()` to stamp `origin`; (5) a remotely-created schedule/webhook fired against a fake `RunManagerPort` that itself applies `admissionRefusal()` is refused on an `unconfined` posture while its locally-created twin still runs; (6) `SqliteRunStore.getSpec()` synthesizes `origin:'local'` on every read-back (the field is never persisted in the `runs` table — a resume is gated by DES-262's door, not this predicate). **Owner ruling preserved, not reopened**: this task does NOT key admission on the workflow VERSION's registering author — only on the TRIGGER's own stored provenance (ADR-086, owner_decision answered 2026-09-23). **[SUPERSEDED 2026-09-25 by TASK-259 — ADR-086 的第三次裁決(P1)正是把這句話反過來:admission 現在也 key 在版本的註冊者身上,兩個來源取 OR。原文保留,因為 TASK-258 是在當時的裁決之下驗收的。]**
- **estimate:** S
- **iter:** v37

### TASK-259 — P1:把「腳本來源」搬到 `workflow_versions.registeredRemote`,刪除 `claim()` 的重新蓋章,接納述詞改為兩個不可變來源取 OR(ARCH-182, ADR-086 第三次裁決, DES-263 第三次修訂, REQ-218)
- **status:** done
- **traces:** ARCH-182, DES-263, ADR-086, REQ-218
...
- **des:** DES-263
- **dod:** `npx tsc --noEmit && npm test` 全綠,且下列每一條都有對應的紅→綠證據:
