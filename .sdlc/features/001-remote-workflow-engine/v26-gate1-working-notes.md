...
## Gate 2 之後的補充裁決(2026-09-08 上午)
| 問題(架構師 owner_decision) | 裁決 |
|---|---|
| 查不到價格的模型:拒絕(`PRICE_UNKNOWN`)還是放行? | **以 0 計費,不拒絕**;紀錄標未定價、可查詢 |
| trigger 啟動的 run 要不要花費上限(D-V2h)? | **(b) 不設**;編寫者建立時已知成本;事後可查詢即可 |
| 原則 | **預算不是一定要設;不設 = 無上限。但一定要可追蹤、可紀錄、可事後查詢以調整。** |
