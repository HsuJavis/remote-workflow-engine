...
### IMPL-122 — auth route wiring: resolvePrincipal + 5 OAuth routes + auth gates on /mcp, /assets/blob/:sha, /assets/manifest; v16: isLoopbackRedirectUri + gcExpired sweep wiring; v17: RFC 7591 DCR (TASK-086, TASK-090, TASK-091)
- **status:** done
- **traces:** TASK-086, TASK-090, TASK-091, DES-092, DES-093, DES-095
- **greens:** UT-095, IT-078 (cases 1-7, 8a-8f, 9a-9c, 10, 11-18), IT-079 (regression), VAL-095 (cases 1-6)
- **files:** src/auth/oauth-metadata.ts, src/auth/token-store.ts, src/auth/auth-service.ts, src/server.ts, vitest.config.ts
- **commit:** (pending)
- **iter:** v17
