...
## Iteration v29d — 第二群:模型頁與系統頁(B18 / B20 / B21 / B22)

### REQ-156 — 模型頁的標頭列(B18)
README §4 的標頭:標題、計數、目錄更新時間、排序提示。目前只有一個裸露的數字。
- **驗收(zh):**「模型清單」、「N 個模型」(篩選時「M / N」)、「目錄更新 <時間>」、「點欄位排序」。
- 目錄更新時間取自 `entries[0].catalogFetchedAt`(`ui/models.js:239` 已在讀它做 digest,未曾顯示)。

### REQ-157 — 系統頁的標頭(B21)
- **驗收(zh):**「系統資源」與「取樣於 <時間>」;時間取自 `/api/system` 的 `sampledAt`。

### REQ-158 — 模型細節面板的背景遮罩(B20)
agent 面板有遮罩(`rweFadeIn`),模型面板沒有 —— **同一份實作的兩個滑入面板彼此不一致**。
- **驗收:** 開啟模型面板時存在一個覆蓋全視窗的遮罩,點它關閉面板,動畫名為 `rweFadeIn`。

### REQ-159 — `fmtBytes` 的四個缺陷(B22)
稽核只記了進位,量測後是四個:

| | 現況 | 應為(交付稿 `fmtBytes`) |
|---|---|---|
| 進位 | 1000 | **1024** |
| 缺值 | `fmtBytes(null) → "null B"`、`undefined → "undefined B"`、`NaN → "NaN B"` | **`'—'`** |
| 單位 | 最大 GB(1.8 TB 顯示成 `1979.1 GB`) | 到 **TB** |
| 精度 | 一律一位小數 | GB 以下取整,GB 起一位小數 |

**第二列是本 ledger 反覆關掉的那個類別**(BF-5/BF-6:字面 `undefined` 上畫面),
不是外觀差異。`fmtBytes` 是純函式且已有單元測試檔,四者一次修。
...
## Iteration v30b — AUDIT-v29 的 24 條 low(實作 22、兩條在動手時改變性質)

REQ-170..185。全部來自獨立稽核的 low 清單;逐條對 README 查證後實作。

**兩條在動手時發現不是稽核描述的那回事:**

- **F27「搜尋框只有 200px」** —— `.home-search{max-width:320px}` 本來就是對的。
  只有 `max-width` 沒有 `width`,輸入框用了瀏覽器預設尺寸(約 20 字元),
  **那個上限從來沒有生效過**。修法是補 `width:100%`,不是改上限。
- **F28「藥丸圓角」** —— 8 處裡只有 3 處該改(`.tag` / `.btn` / `.run-chip` / `.event-kind` / `.rwe-connection` 這組 tag-btn 形狀)。
  另外 4 處是真正的圓點與滑桿把手;README 明文「pulsing 8 px accent **dot**」——
  那是形狀,不是 radius token。

**第九個抄自實作的 oracle,而且是它指出我也改錯了:**
`dashboard-spec.ts:168` 釘著 `data-run-chip` 的 `border-radius: 100px`。
它紅掉時顯示 `got "2px"` —— 我把 chip 改成了 `--radius-sm`,
而 README §2 說 run chip 是「outlined `.btn`s」,該用 `--radius-md` 3px。
**若那一列當初釘的是 3px,我第一次就會改對。**

### 幾條比稽核描述更有內容的

- **REQ-173(F19)** v29 c2 的註解證成的是「ink 不隨色相」,不是那個**值**。
  README:83 的文字色是 `#e2e6e5`,實作是 `#e7e9ec` —— 稽核抓到的正是這個區別。
- **REQ-170(F25)**「全部 (10)」不只是數字不同 —— 它**完全不理會搜尋框**,
  四張卡符合 `probe` 時仍顯示 10,描述的是使用者沒在看的集合。
  `all` 改為 `running + registered`(設計自己的定義),並對**過濾後**的清單計算。
  連帶重新推導 `UT-246` 的 `all: 3` —— 那是 `cards.length`,實作自己的定義。
- **REQ-184(F8)** 空的基準分數區塊被隱藏,而 Won't-have D2 讓「空」是**唯一**會出現的情況 ——
  讀者分不出「這個模型沒有基準分數」與「這個面板不再渲染了」。
- **REQ-179(F12)** 節點列的 `—` 夾在兩個真實數字之間會被讀成一個「值」;設計是直接略過那一段。

---

## Iteration v31 — REQ-186(R30-A1:未量到的用量必須是缺席,不是零)

**來源:** R30-A1。兩次獨立稽核**都**在「限制」一節寫了同一句話 —— 這台引擎沒有進行中的
工作流,所以執行中的狀態只驗到 CSSOM。真跑一個三階段工作流把那些狀態逼出來之後,
那次真跑本身暴露了兩次稽核都看不到的東西。

### 驗收

| 狀態 | `tokens` | 理由 |
|---|---|---|
| `queued` / `running` | **缺席** | 呼叫還沒回來,沒有任何測量發生過 |
| `failed` | 零 | DES-188:「a failed call moves no counter」—— 那是**測量結果** |
| `refused` | 零 | 終端狀態,沒有人在等它的數字 |
| `done` | 真值 | 不變 |

**這條線是「還沒量」與「量到了零」,不是一條通則。** UT-277 的四個案例正好釘住它:
兩個 RED(running/queued)、兩個從一開始就綠(failed/done)。

### 兩個產生器必須同時改

`run-store.ts` 的分支 (4) 與 `agent-executor.ts` 的 `markQueued`/`markRunning`。
前者的註解明寫兩者「byte-identical」—— 只改一邊,重啟前後的同一筆紀錄就會不一致。

### 我上一輪對修法的判斷是錯的,已更正

登記 R30-A1 時我寫「客戶端對 `undefined` 早就會顯示 `—`,所以只要改引擎」。查證後:

```
sumTokens(undefined) → 0            把缺席映成了零
fmtTok(0)            → "0"
fmtTok(undefined)    → "undefined"  它根本沒有缺席分支
```

**最後一行是同一個字面-undefined 類別,只是從未被觸發** —— 每個呼叫端都剛好遞給它一個
零填充的物件。**若只改引擎,畫面會從 `0 tok` 變成 `undefined tok`。** 兩側一起改。
...
### REQ-217 — 消除清單路徑的擴展懸崖,且不改變數字的語意(v36 追加,業主 2026-09-22 裁決 K5)
