...
- **儀表板**：`GET /dashboard` —— 深色系操作介面（畫面右上角三段式主題切換：跟隨系統／淺色／
  ...
  **引擎連不上的時候**：畫面右上角的連線燈會從「連線中」變成「示範資料 / Demo data」，並且用示範
  內容把畫面填滿（**不是空白**，方便看清楚少了什麼），同時整頁上方會多一條橫幅明講「這是示範內容」
  ——「示範資料」和「連線中」兩個字樣不會同時出現，不會誤以為是真資料。引擎恢復連線後，下一次
  自動更新就會自動切回真資料，不用重新整理頁面。
