...
- **儀表板**：`GET /dashboard` —— 深色系操作介面（畫面右上角三段式主題切換：跟隨系統／淺色／
  ...
  **引擎連不上的時候**：畫面右上角的連線燈會從「連線中」變成「示範資料 / Demo data」，同時整頁
  ...
  為是真資料。工作流程首頁、模型分頁、系統分頁這三處會用示範內容填滿畫面（方便看清楚少了什麼）。
  **有兩個地方目前還沒有示範內容**（見下方「已知限制」）：工作流程詳情頁、問題分頁——這兩處在
  引擎斷線後會停在斷線前最後一次看到的畫面，不會更新也不會清空。引擎恢復連線後，下一次自動更新
  就會自動切回真資料，不用重新整理頁面。
...
- **示範模式下兩個分頁沒有示範內容**：引擎斷線後，工作流程詳情頁（`/dashboard/workflow/<name>`）
  和問題分頁會停在斷線前最後一次看到的畫面（不更新、不清空、也不特別提示），因為這兩處背後的
  API（`/api/workflows/:name/describe`、`/api/issues`）沒有可用的示範資料可以填（前者在測試
  fixture 裡沒有欄位預言，後者的真實資料含 GitHub 網址，本專案的安全守則禁止在前端程式碼裡出現
  網址字串）——這是刻意的取捨（不造假內容），但「這裡沒有示範資料」的明確提示字樣目前還沒做，
  待補（[issue 追蹤中](https://github.com/HsuJavis/remote-workflow-engine/issues)）。
