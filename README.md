...
- **儀表板**：`GET /dashboard` —— 深色系操作介面（畫面右上角三段式主題切換：跟隨系統／淺色／
  ...
  - **工作流程**（首頁，品牌字樣「工作流引擎 / Workflow Engine」在導覽列上）：卡片依「執行中 /
    ...
    一樣看得到（不會因為沒登入就變空白）。
  - **模型**：目錄表（provider / model / capability / stability / costLevel / modalities 六欄），
    本版不支援排序、篩選，也沒有點列滑出細節面板。
  - **系統**：資源表格（CPU 核心數、負載平均、CPU 使用率、記憶體、磁碟、取樣時間，共六列），本版
    沒有卡片版面，也沒有處理程序表。
