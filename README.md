...
- **儀表板**：`GET /dashboard` —— 深色系操作介面（畫面右上角三段式主題切換：跟隨系統／淺色／
  ...
  **泳道圖**：每個 `phase()` 是一條直向泳道，agent 節點依派工順序排列、用曲線互相連接，顏色會隨
  ...
  努力程度、token 用量/費用/耗時。圖支援滑鼠滾輪縮放、按住拖曳平移，`Fit` 按鈕重置；每 3 秒自動
  更新一次，已經做好的縮放/平移不會被重設；分頁切到背景時會暫停更新，切回來才繼續打。
  ...
  **agent 細節面板**：顯示這個 agent 的模型 / 努力程度 / 逾時 / token 用量（含快取讀寫）/ 費用，
  ...
  按 Esc 或點背景可以關閉面板。**目前只有透過該次 run 的專屬網址 `/dashboard/<runId>`（把執行歷史
  表裡 mono 字型那欄的 run ID 貼到網址列）點節點才會打開這個面板**；在工作流程詳情頁的泳道圖上點
  節點暫時還不會有反應（見下方「已知限制」）。
  ...
  /dashboard  （深色殼：主題/語言/連線燈/品牌，四個分頁）
   ...
   │                                             │ 複製某次 run 的 ID
   │                                             ▼
   │                                  /dashboard/<runId>（同一張泳道圖，點節點會打開面板）
   │                                             │ 點 agent 節點
   │                                             ▼
...
- **目前已知、尚未修復的缺陷**（詳細指令與輸出見 `DEPLOY.md` §6）：
  ...
  2. **工作流程詳情頁的泳道圖點節點不會打開 agent 細節面板**（只有 `/dashboard/<runId>` 這個
     專屬網址上點節點才有反應——見上方「儀表板」小節）。已回報，下一輪修復。
